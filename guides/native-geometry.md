...
OCEx exposes native shapes and operations. Smith adds deferred recipes, selectors,
assemblies, and printable bundles on top. Use OCEx directly when you need native
...
| Profiles   | `edge/2`, `circle/1`, `arc/6`, `spline/1,2`, `bezier/1`, `wire/1`, `face/1`                                                                                               |
...
| Transforms | `translate/2`, `rotate/4`, `scale/2`, `mirror/3`                                                                                                                          |
| Finishing  | `fillet/3`, `chamfer/3`, `clean/1`                                                                                                                            |
| Topology   | `vertices/1`, `edges/1`, `wires/1`, `faces/1`, `shells/1`, `solids/1`, `shape_type/1`, `same?/2`                                                              |
| Inspection | `valid?/1`, `volume/1`, `area/1`, `length/1`, `center_of_mass/1`, `bounds/1`, `point/1`, `edge_info/1`, `face_info/1`, `edge_sample/2`, `distance_to_point/2`, `closest_points/2` |
| Exchange   | `to_brep/1`, `from_brep/1`, `read_step/1`, `write_step/2`, `write_stl/2,3,4`, `mesh/1,2,3`, `drawing/4,5`, `polylines/1,2,3`, `version/0`                                                       |
...
## Distance and feature measurements

`closest_points/2` returns material distance and a witness point on each shape.
Intersecting or contained solids have distance zero; use common volume to distinguish
interference from contact. Equal-distance ties may return a different witness pair
on another revision. `edge_info/1` exposes exact circular centers and radii for
feature measurements, rather than inferring them from mesh vertices.

```elixir
{:ok, first} = OCEx.box(10, 10, 10)
{:ok, second} = OCEx.translate(first, {13, 0, 0})
{:ok, separation} = OCEx.closest_points(first, second)
true = abs(separation.distance - 3) < 1.0e-7
```
...
`bezier(points)` constructs a polynomial Bézier curve from control points. Unlike
an interpolating spline, it generally touches only its first and last control
points and remains inside their control polygon's convex hull.
